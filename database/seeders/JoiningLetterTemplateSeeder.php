...
class JoiningLetterTemplateSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $languages = [
            'ar' => 'خطاب الانضمام',
            'da' => 'Tiltrædelsesbreve',
            'de' => 'Beitrittsschreiben',
            'en' => 'Joining Letter',
            'es' => 'Carta de Incorporación',
            'fr' => 'Lettre d\'Adhésion',
            'he' => 'מכתב הצטרפות',
            'it' => 'Lettera di Adesione',
            'ja' => '入社通知書',
            'nl' => 'Toetredingsbrief',
            'pl' => 'List Dołączenia',
            'pt' => 'Carta de Adesão',
            'pt-BR' => 'Carta de Adesão',
            'ru' => 'Письмо о Присоединении',
            'tr' => 'Katılım Mektubu',
            'zh' => '入职信'
        ];
        ...
        foreach ($companies as $company) {
            foreach ($languages as $code => $title) {
                try {
                    JoiningLetterTemplate::updateOrCreate(
                        [
                            'language' => $code,
                    ...
                    $this->command->error('Failed to create Joining Letter template for language: ' . $code . ' and company: ' . $company->name);
                    continue;
                }
            }
        }

        $this->command->info('JoiningLetterTemplate seeder completed successfully!');
    }
}