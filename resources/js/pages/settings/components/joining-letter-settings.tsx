...
interface JoiningLetterTemplate {
  id: number;
  language: string;
  content: string;
  variables?: string[];
...
interface JoiningLetterSettingsProps {
  templates?: JoiningLetterTemplate[];
}

export default function JoiningLetterSettings({ templates = [] }: JoiningLetterSettingsProps) {
  ...
  const defaultTemplate = `
    ...
    <p>We are pleased to welcome you to {company_name} as {designation}.</p>
    
    <p>Joining Date: <strong>{joining_date}</strong></p>
    <p>Salary: <strong>{salary}</strong></p>
    <p>Department: <strong>{department}</strong></p>
    
    <p>We look forward to your valuable contribution to our company's success.</p>
  ...
  const handleSave = async () => {
    ...
    const template = templates.find(t => t.language === selectedLanguage);
    try {
      router.post(route('settings.joining-letter.update'), {
        ...
        onError: (errors) => {
          if (typeof errors === 'string') {
            ...
            toast.error(t('Failed to update Joining Letter template: {{errors}}', { errors: Object.values(errors).join(', ') }));
          }
        },
      ...
      setIsLoading(false);
      toast.error(t('Failed to update Joining Letter template'));
    }
  };
  ...
  return (
    <SettingsSection
      ...
      description={t("Configure Joining Letter templates for different languages")}
      action={
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{t("Design your Joining Letter template using the rich text editor")}</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
                  <div className="w-full">
                    <RichTextEditor
                      ...
                      content={content}
                      onChange={setContent}
                      placeholder={t('Enter Joining Letter template content...')}
