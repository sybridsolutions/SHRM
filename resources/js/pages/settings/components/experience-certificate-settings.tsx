...
interface ExperienceCertificateTemplate {
  id: number;
  language: string;
  content: string;
  variables?: string[];
...
interface ExperienceCertificateSettingsProps {
  templates?: ExperienceCertificateTemplate[];
}

export default function ExperienceCertificateSettings({ templates = [] }: ExperienceCertificateSettingsProps) {
  ...
  const getPlaceholders = () => {
    ...
    return [
      ...
      { key: 'Joining Date', label: '{joining_date}' },
      { key: 'Leaving Date', label: '{leaving_date}' }
    ];
  };
  ...
  const defaultTemplate = `
    ...
    <p>This is to certify that <strong>{employee_name}</strong> was employed with {company_name} as {designation} from {joining_date} to {leaving_date}.</p>
    
    <p>During the period of employment, the above-mentioned employee demonstrated excellent performance and high professional skills. He/She was a dedicated and responsible employee.</p>
  ...
  const handleSave = async () => {
    ...
    const template = templates.find(t => t.language === selectedLanguage);
    try {
      router.post(route('settings.experience-certificate.update'), {
        ...
        onError: (errors) => {
          if (typeof errors === 'string') {
            ...
            toast.error(t('Failed to update Experience Certificate template: {{errors}}', { errors: Object.values(errors).join(', ') }));
          }
        },
      ...
      setIsLoading(false);
      toast.error(t('Failed to update Experience Certificate template'));
    }
  };
  ...
  return (
    <SettingsSection
      ...
      description={t("Configure Experience Certificate templates for different languages")}
      action={
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{t("Design your Experience Certificate template using the rich text editor")}</p>
                        </TooltipContent>
                      </Tooltip>
                    </TooltipProvider>
                  </div>
                  <div className="w-full">
                    <RichTextEditor
                      ...
                      content={content}
                      onChange={setContent}
                      placeholder={t('Enter Experience Certificate template content...')}
